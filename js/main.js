...
function Reset(error = false, errorMsg = "") {
    ...
    var ctx_main_canvas = canvas.getContext("2d");
    ctx_main_canvas.clearRect(0, 0, ctx_main_canvas.canvas.width,
        ctx_main_canvas.canvas.height);
    DrawBGCanvas();
}

function DrawCars() {
    var bg_canvas = document.getElementById('bgCanvas');
    var ctx_bg_canvas = bg_canvas.getContext('2d');
    var scaleFactor = ctx_bg_canvas.canvas.height / STANDARD_SCALE;
    for (x = 0; x < CAR_POS.length; x++) {
        if (carOutLoaded) {
            ctx_bg_canvas.drawImage(car_out_blk, Math.floor((CAR_POS[x][0] - 1) *
                scaleFactor), Math.floor((CAR_POS[x][1] - 1) *
                scaleFactor), Math.floor(18 * scaleFactor), Math.floor(
                18 * scaleFactor));
        }
        ctx_bg_canvas.drawImage(car, Math.floor(CAR_POS[x][0] * scaleFactor),
            Math.floor(
                CAR_POS[x][1] * scaleFactor), Math.floor(16 * scaleFactor),
            Math.floor(16 * scaleFactor));
    }
}
...
function DrawBGCanvas() {
    ...
    var ctx_bg_canvas = bg_canvas.getContext('2d');
    if (pathDone) {
        ctx_bg_canvas.drawImage(bgLetters, 0, 0, bg_canvas.width, bg_canvas.height);
    } else {
        ctx_bg_canvas.drawImage(bgLines, 0, 0, bg_canvas.width, bg_canvas.height);
    }
    DrawCars();
}
...
function Resize(scale) {
    ...
    // ctx_main_canvas.canvas.width = ctx_main_canvas.canvas.height;

    // ctx_bg_canvas.canvas.height = container.clientHeight;
    // ctx_bg_canvas.canvas.width = ctx_bg_canvas.canvas.height;
    ...
    if (scale) {
        ...
        if (pathDone) {
            ...
            ctx_main_canvas.clearRect(0, 0, ctx_main_canvas.canvas.width,
                ctx_main_canvas.canvas.height);
            setTimeout(function() {
                if (Date.now() >= lastScaled + 400 && Date.now() >=
                    lastResizeDraw + 400) {
                    //Draw the path!
                    DrawPath(true);
                }
...
function Scale() {
    ...
    container.style.marginTop = (window.innerHeight - container.clientHeight) /
        2;
    canvascont.style.width = ctx_main_canvas.canvas.width;
    canvascont.style.height = ctx_main_canvas.canvas.height;
    // canvascont.style.left = leftbar.clientWidth;
}
...
function DrawArrow(fromx, fromy, tox, toy) {
    ...
    var ctx_main_canvas = c.getContext("2d");
    var headlen = 10;
    ...
    ctx_main_canvas.beginPath();
    ctx_main_canvas.moveTo(fromx, fromy);
    ctx_main_canvas.lineTo(tox, toy);
    ctx_main_canvas.strokeStyle = "#FFFFFF";
    ctx_main_canvas.lineWidth = 6;
    ctx_main_canvas.stroke();

    //starting a new path from the head of the arrow to one of the sides of the point
    ctx_main_canvas.beginPath();
    ctx_main_canvas.moveTo(tox, toy);
    ctx_main_canvas.lineTo(tox - headlen * Math.cos(angle - Math.PI / 7), toy -
        headlen *
    ...
    ctx_main_canvas.lineWidth = 6;
    ctx_main_canvas.stroke();
    ctx_main_canvas.fillStyle = "#FFFFFF";
    ctx_main_canvas.fill();
}
...
function DrawClick(x, y) {
    ...
    ctx_main_canvas.beginPath();
    ctx_main_canvas.arc(x, y, 6, 0, 2 * Math.PI, false);
    ctx_main_canvas.fillStyle = '#FFFFFF';
    ctx_main_canvas.strokeStyle = '#000000';
    ctx_main_canvas.stroke();
    ctx_main_canvas.fill();
}
...
function DrawPath(resizeDraw = false) {
    ...
    if (intersect1 == null || intersect2 == null || intersect3 == null ||
        intersect4 == null) {
...
function UpdateLine() {
    ...
    ctx_main_canvas.beginPath();
    ctx_main_canvas.moveTo(lineDraw1[0], lineDraw1[1]);
    ctx_main_canvas.lineTo(lineDraw2[0], lineDraw2[1]);

    ctx_main_canvas.strokeStyle = '#000000';
    ctx_main_canvas.lineWidth = 8;
    ctx_main_canvas.stroke();
    ctx_main_canvas.strokeStyle = '#FFFFFF';
    ctx_main_canvas.lineWidth = 6;
    ctx_main_canvas.stroke();
...
function HandleDown(event) {
    ...
    if (arrow) {
        FinishPath(downEvent.offsetX, downEvent.offsetY, moveEvent.offsetX,
            moveEvent.offsetY)
    } else if (!pathDone) {
...
function HandleMove(event) {
    ...
    if (new Date().getTime() - lastDown > 20 && dragging && pathDone == false &&
        clicked == false) {
        arrow = true;
...
function DisplayCircles() {
    ...
    for (i = 0; i < count; i++) {
        ...
        str = str + "<div class=" + circleclass + "><div class='dps'>" + dps +
            "</div><div class='dpsfooter'>DPS</div><div class='closetime'>";
...
function UpdateHealth() {
    if (circles.length > 0) {
        ...
        healthbarcont.innerHTML = "<progress id='healthbar' class='" + myclass +
            "' value='" + lowestHP.toString() +
            "' max='100'></progress><div id='healthtext'>LOWEST HEALTH TO USE FIRST AID</div>";
    }
}
    ...
    if (pathDone) {
        ...
        if (circles.length > 0) {
            ...
            if (circles[i].uptime <= 0) {
                ...
                var height = ctx_main_canvas.canvas.height;

                //ctx_main_canvas.clearRect(0, 0, ctx_main_canvas.canvas.width, ctx_main_canvas.canvas.height);

                var scaleFactor = height / STANDARD_SCALE;

                setTimeout(function() {
                    CircleAnim(els, a_time, animDate, 0, scaleFactor)
                }, 5);
                setTimeout(function() {
                    DisplayCircles();
                }, a_time);
...
function CircleAnim(els, a_time, animDate, pos, scaleFactor) {
    ...
    setTimeout(function() {
        CircleAnim(els, a_time, animDate, pos, scaleFactor)
    }, 5);
}
...
function CalcDists(resizeDraw) {
    if (pathDone) {
        ...
        // var height = ctx_main_canvas.canvas.height;
        // var width = ctx_main_canvas.canvas.width;
...
function PostPathUpdate(resizeDraw = false) {
    ...
    //             {
    //                 DrawPixel(ctx_main_canvas, x, y, c1[0], c1[1], c1[2], c1[3]);
    //             }
    //             else if(dist <= CHUTE_DIST[1] * scaleFactor)
    //             {
    //                 DrawPixel(ctx_main_canvas, x, y, c2[0], c2[1], c2[2], c2[3]);
    //             }
    //         }
    //     }
    // }
    ...
    if (newEnd1[0] > -.1 && newEnd1[0] < .1) {
        newEnd1 = math.intersect(newEnd1, newEnd2, [-200, 0], [-200, height +
            200
        ]);
    }
    if (newEnd1[0] > width - 1 && newEnd1[0] < width + 1) {
        newEnd1 = math.intersect(newEnd1, newEnd2, [width + 200, 0], [width +
            200, height + 200
        ]);
    }
    if (newEnd1[1] > -.1 && newEnd1[1] < .1) {
        newEnd1 = math.intersect(newEnd1, newEnd2, [0, -200], [width + 200, -
            200
        ]);
    }
    if (newEnd1[1] > height - 1 && newEnd1[1] < height + 1) {
        newEnd1 = math.intersect(newEnd1, newEnd2, [0, height + 200], [width +
            200, height + 200
        ]);
    }

    if (newEnd2[0] > -.1 && newEnd2[0] < .1) {
        newEnd2 = math.intersect(newEnd1, newEnd2, [-200, 0], [-200, height +
            200
        ]);
    }
    if (newEnd2[0] > width - 1 && newEnd2[0] < width + 1) {
        newEnd2 = math.intersect(newEnd1, newEnd2, [width + 200, 0], [width +
            200, height + 200
        ]);
    }
    if (newEnd2[1] > -.1 && newEnd2[1] < .1) {
        newEnd2 = math.intersect(newEnd1, newEnd2, [0, -200], [width + 200, -
            200
        ]);
    }
    if (newEnd2[1] > height - 1 && newEnd2[1] < height + 1) {
        newEnd2 = math.intersect(newEnd1, newEnd2, [0, height + 200], [width +
            200, height + 200
        ]);
    }
    ...
    ctx_main_canvas.beginPath();
    ctx_main_canvas.moveTo(newEnd1[0], newEnd1[1]);
    ctx_main_canvas.lineTo(newEnd2[0], newEnd2[1]);

    ctx_main_canvas.strokeStyle = 'rgba(0, 0, 100, 0.17)';
    if (lw1 > CHUTE_DIST[1] * scaleFactor * 2) {
        lw1 = CHUTE_DIST[1] * scaleFactor * 2
    }
    ctx_main_canvas.lineWidth = lw1;
    ctx_main_canvas.stroke();
    ...
    ctx_main_canvas.strokeStyle = 'rgba(255, 200, 0, 0.2)';
    lw = lw_og;
    if (lw2 > CHUTE_DIST[0] * scaleFactor * 2) {
        lw2 = CHUTE_DIST[0] * scaleFactor * 2;
    }
    ctx_main_canvas.lineWidth = lw2;
    ctx_main_canvas.stroke();
    ...
    for (x = 0; x < CAR_POS.length; x++) {
        ...
        if (dist <= distLimit && dist > lastDistLimit) {
            ...
            if (dist <= CHUTE_DIST[0] * scaleFactor + 5) {
                ctx_bg_canvas.drawImage(car_out_org, Math.floor((CAR_POS[x][0] -
                        1) *
                    scaleFactor), Math.floor((CAR_POS[x][1] - 1) *
                    scaleFactor), Math.floor(18 * scaleFactor), Math.floor(
                    18 * scaleFactor));
                ctx_bg_canvas.drawImage(car, Math.floor(CAR_POS[x][0] *
                        scaleFactor),
                    Math.floor(CAR_POS[x][1] * scaleFactor), Math.floor(16 *
                        scaleFactor), Math.floor(16 * scaleFactor));
            } else if (dist <= CHUTE_DIST[1] * scaleFactor + 5) {
                ctx_bg_canvas.drawImage(car_out_blue, Math.floor((CAR_POS[x][0] -
                        1) *
                    scaleFactor), Math.floor((CAR_POS[x][1] - 1) *
                    scaleFactor), Math.floor(18 * scaleFactor), Math.floor(
                    18 * scaleFactor));
...
// 	if(bgLoaded)
// 	{
// 		ctx_main_canvas.drawImage(bg, 0, 0, canvas.width, canvas.height);
// 		DrawHeatAtPoint(ctx_main_canvas, 50, 50, 50)
// 	}
